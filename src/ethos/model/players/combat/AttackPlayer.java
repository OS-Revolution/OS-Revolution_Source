	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		case MELEE:
			...
			case 0: // Accurate
				...
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige()));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige())), 0, false);
				break;
			case 1: // Block
				skills.add(Skill.DEFENCE.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige()));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige())), 1, false);
				break;
			case 2: // Aggressive
				skills.add(Skill.STRENGTH.getId());
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige()));
				c.getPA().addSkillXP((int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige())), 2, false);
				break;
			case 3: // Controlled
				int split = (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige())) / 3;
				experience += (int) Math.ceil(damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.MELEE_EXP_RATE * c.prestige()));
				for (int i = 0; i < 3; i++) {
		...
		case RANGE:
			skills.add(Skill.HITPOINTS.getId());
			experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige())) / 3;
			c.getPA().addSkillXP((int) ((damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige())) / 3), 3, false);
			switch (c.fightMode) {
			case 0: // Accurate
			case 2: // Rapid
				skills.add(Skill.RANGED.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige()));
				c.getPA().addSkillXP((int) (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige())), 4, false);
				break;
			case 1: // Block
			case 3:
				skills.add(Skill.RANGED.getId());
				skills.add(Skill.DEFENCE.getId());
				experience += (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige())) + (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE) / 2);
				c.getPA().addSkillXP((int) (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige())), 1, false);
				c.getPA().addSkillXP((int) (damage * (c.getMode().getType().equals(ModeType.OSRS) ? 4 : Config.RANGE_EXP_RATE * c.prestige()) / 2), 4, false);
				break;
			}
			break;
		case MAGE:
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (attacker.magicDef) {
					attacker.getPA().addSkillXP((int) (damage * (attacker.getMode().getType().equals(ModeType.OSRS) ? 3 : Config.MELEE_EXP_RATE * attacker.prestige()) / 3), 1, false);
