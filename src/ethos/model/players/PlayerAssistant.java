...
public class PlayerAssistant {
    ...
    public void sendBonusXp(int skillId, int xp) {
        if (c.getOutStream() != null && c != null) {
            c.getOutStream().createFrame(135);
            c.getOutStream().writeByte(skillId);
            c.getOutStream().writeWord(xp);
            c.flushOutStream();
        }
    }
...
//			c.getOutStream().writeByte(c.getContext().getPlayerSaveData().getBonusXp().containsKey(skillNum) ?
//					c.getContext().getPlayerSaveData().getBonusXp().get(skillNum).value() : 0);
            c.flushOutStream();
        }
    }
