...
public class Crafting {
	...
	private void craftHides(int id) {
		for (int j = 0; j < amount; j++) {
			...
			c.getPA().addSkillXP((int) (exp * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.CRAFTING_EXPERIENCE * c.prestige())), Player.playerCrafting, true);
		}
		resetCrafting();
	}
			...
			Server.getEventHandler().submit(new Event<Player>("skilling", c, 2) {
				...
				public void execute() {
					...
					if (attachment.getItems().playerHasItem(g.uncut)) {
						...
						attachment.getPA().addSkillXP((int) (g.experience * (attachment.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.CRAFTING_EXPERIENCE * c.prestige())), Skill.CRAFTING.getId(), true);
