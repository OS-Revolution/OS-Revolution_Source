...
public class Smelting {
	...
	private static void appendDelay(Player c, String usage) {
		...
		double percentOfXp = c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE * c.prestige()) / 2;
		c.prestigeNumber = 13;
		...
		if (c.smeltAmount > 0) {
			...
			if (hasItems) {
				...
				if (c.bar.twoOres()) {
					...
					c.getItems().addItem(c.bar.getBar(), 1);
					c.getPA().addSkillXP((int) (Objects.equals(usage, "INFERNAL") ? c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE / 2) : c.bar.getExp() * (c.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.SMITHING_EXPERIENCE * c.prestige()) + (goldSmithGaunts ? percentOfXp : 0)), Player.playerSmithing, true);
				} else {
					if (!Objects.equals(usage, "INFERNAL")) {
						c.startAnimation(899);
					}
