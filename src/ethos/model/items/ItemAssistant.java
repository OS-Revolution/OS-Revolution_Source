...
//					}
    ...
    public void getBonus() {
        for (int i = 0; i < c.playerEquipment.length; i++) {
            if (c.playerEquipment[i] > -1) {
                ...
                try {
                    Equipment equipmentContext = EquipmentCache.getInstance().read(equipmentItem);
                    if (equipmentContext != null) {
                        c.playerBonus[0] += equipmentContext.getAttackBonus()[0];
                        c.playerBonus[1] += equipmentContext.getAttackBonus()[1];
                        c.playerBonus[2] += equipmentContext.getAttackBonus()[2];
                        c.playerBonus[3] += equipmentContext.getAttackBonus()[3];
                        c.playerBonus[4] += equipmentContext.getAttackBonus()[4];

                        c.playerBonus[5] += equipmentContext.getDefenceBonus()[0];
                        c.playerBonus[6] += equipmentContext.getDefenceBonus()[1];
                        c.playerBonus[7] += equipmentContext.getDefenceBonus()[2];
                        c.playerBonus[8] += equipmentContext.getDefenceBonus()[3];
                        c.playerBonus[9] += equipmentContext.getDefenceBonus()[4];

                        c.playerBonus[10] += equipmentContext.getMeleeStrengthBonus();
                        c.playerBonus[11] += equipmentContext.getPrayerBonus();
                    }
    ...
    public void sendWeapon(int Weapon, String WeaponName) {
        ...
        if (WeaponName.equals("Unarmed")) {
            ...
            c.getPA().sendFrame126(WeaponName, 331);
        } else if (WeaponName.equals("Scythe of vitur")) {
            c.setSidebarInterface(0, 8460); // jab, swipe, fend
...
//        }
    ...
    public boolean is2handed(String itemName, int itemId) {
        if (EquipmentCache.getInstance().read(itemId) != null && EquipmentCache.getInstance().read(itemId).getSlot() == 11) {
            return true;
        }
...
//		}
    ...
    public static String getItemName(int itemId) {
        ...
        return ItemIdContextLoader.getInstance().read(itemId) != null ? ItemIdContextLoader.getInstance().read(itemId).getName() :
                (itemList.itemName == null ? "Unarmed" : itemList.itemName);
    }
