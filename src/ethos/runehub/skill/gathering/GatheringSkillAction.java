...
public abstract class GatheringSkillAction extends SkillAction {
	...
	public void onTick() {
		...
		if (this.isSuccessful(
				(int) (targetedNodeContext.getNode().getMinRoll()
						* this.getActor().getSkillController().getGatheringSkill(this.getSkillId()).getPowerBonus()),
				(int) (targetedNodeContext.getNode().getMaxRoll()
						* this.getActor().getSkillController().getGatheringSkill(this.getSkillId()).getPowerBonus()))) {
			this.onGather();
		}
	}
	...
	protected void onActionStart() {
		this.getActor()
				.startAnimation(this.getActor().getContext().getPlayerSaveData().getSkillAnimationOverrideMap()
						.containsKey(this.getSkillId())
								? this.getActor().getContext().getPlayerSaveData().getSkillAnimationOverrideMap().get(
										this.getSkillId())
								: tool.getAnimationId());
		...
		this.getActor().getSkillController().getFishing()
				.setEfficiency(this.getGetBestAvailableTool().getBaseEfficiency());
		this.getActor().getSkillController().getFishing()
				.setGains(this.getGetBestAvailableTool().getXpGainMultiplier());
	}
	...
	protected void validateItemRequirements() {
		Preconditions.checkArgument(this.getActor().getSkillController().getGatheringSkill(this.getSkillId())
				.getGetBestAvailableTool() != null, "You do not have a valid tool.");
	}

	@Override
	protected void validateLevelRequirements() {
		Preconditions.checkArgument(
				PreconditionUtils.isTrue(this.getActor().getSkillController().getLevel(this.getSkillId()) >= this
						.getTargetedNodeContext().getNode().getLevelRequirement()),
				"You need a $" + RunehubUtils.getSkillName(this.getSkillId()) + " level of at least $"
						+ this.getTargetedNodeContext().getNode().getLevelRequirement() + " to do this.");
	...
	protected void validateWorldRequirements() {
		Preconditions
				.checkArgument(
						PreconditionUtils
								.isFalse(Server.getGlobalObjects().exists(this.getRenewableNode().getDepletedNodeId(),
										targetedNodeContext.getX(), targetedNodeContext.getY())),
						"This node is depleted.");
	}

	@Override
	protected void validateInventory() {
		Preconditions.checkArgument(this.getActor().getItems().freeSlots() >= 1,
				"You must have at least $" + 1 + " free inventory slot to do this.");
	}
	...
	protected void addItems() {
		...
		LootTableLoader.getInstance().read(this.getTargetedNodeContext().getNode().getGatherableItemTableId())
				.roll(this.getActor().getAttributes().getMagicFind()).forEach(loot -> {
	...
	protected boolean depleteNode() {
		...
		final RenewableNode node = RenewableNodeLoader.getInstance()
				.read(this.getTargetedNodeContext().getNode().getId());
		final int baseMinRoll = node.getDepletionMinRoll();
		final int playerBaseRoll = Skill.SKILL_RANDOM
				.nextInt(this.getActor().getSkillController().getGatheringSkill(this.getSkillId()).getDepletionOdds());
		final double minRollModifier = this.getActor().getSkillController().getGatheringSkill(this.getSkillId())
				.getEfficiencyBonus();
...
//    }
	...
	protected void onRespawn() {
		int face = 0;
		Optional<WorldObject> worldObject = Region.getWorldObject(targetedNodeContext.getNodeId(),
				targetedNodeContext.getX(), targetedNodeContext.getY(), targetedNodeContext.getZ());
		if (worldObject.isPresent()) {
			face = worldObject.get().getFace();
		}
		Server.getGlobalObjects()
				.add(new GlobalObject(this.getRenewableNode().getDepletedNodeId(), targetedNodeContext.getX(),
						targetedNodeContext.getY(), targetedNodeContext.getZ(), face, 10,
						this.getRenewableNode().getRespawnTime(), targetedNodeContext.getNodeId()));
	}
