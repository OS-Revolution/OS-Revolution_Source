	...
	public static void execute(Player player, int objectId) {
		for (RUNECRAFTING_DATA data : RUNECRAFTING_DATA.values()) {
			...
			if (data.getObjectId() == objectId) {
				...
				player.getPA().addSkillXP((int) (((double) (data.getExperience()) * count) * (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.RUNECRAFTING_EXPERIENCE * player.prestige()) + (player.getItems().isWearingItem(20008) ? percentOfXp : 0)), 20, true);
